...
//  October 2008
//
//  This is the source file of the reference implementation of SWIFFTX hash function.
...
//  algorithm. Note that this implementation is not optimized for speed.
//
///////////////////////////////////////////////////////////////////////////////////////////////
#include "SWIFFTX.h"
...
// The number of DFTS performed on the input
#define M (SWIFFTX_INPUT_BLOCK_SIZE / 8)
...
// - output: the input array encoded in base 256, excluding the carry bit.
//
// Returns:
// - The carry bit (MSB).
int16_t TranslateToBase256(int32_t input[EIGHTH_N], BitSequence output[EIGHTH_N]);

// Generates the table of the powers of OMEGA in the field.
void CalcOmegaPowers();

// Reverses a bit-string.
//
// Parameters:
// - index: the input index to reverse. Is of size LOGN bits.
//
// Returns:
// - The index value with bits reversed.
int BitReverse(int index);

// Initialized the data we use during SWIFFTX operation.
...
int16_t TranslateToBase256(int32_t input[EIGHTH_N], BitSequence output[EIGHTH_N])
{
	...
	for (i = (EIGHTH_N / 2) - 1; i > 0; --i)
	{
		...
		for (j = i - 1; j < (EIGHTH_N / 2) - 1; ++j) 
		{
			...
			int32_t temp = pairs[j] + pairs[j + 1] + (pairs[j + 1] << 9);
			pairs[j] = temp & 0xffff;
			pairs[j + 1] += (temp >> 16);
		}
	}
	...
	for(i = 0; i < EIGHTH_N; i += 2) 
	{
		output[i] = (BitSequence) (pairs[i >> 1] & 0xff);
		output[i + 1] = (BitSequence) ((pairs[i >> 1] >> 8) & 0xff);
	}
...
int BitReverse(int index) 
{
	...
	for (bit = 0; bit < LOGN; ++bit, index >>= 1) 
	{
...
{ 
	int i, j;
	uint8_t carry = 0;
	const BitSequence *in = input;
	int32_t result[N] = {0};

	for (j = 0; j < m; ++j, in += 8, a += 64)
	{
		// Convert N/8 input chars block to an array of N bits:
		int inputBits[N];
		int reversedInputBits[N];
		int dft[N];
		
		for (i = 0; i < N; ++i) 
		{
			inputBits[i] = (in[i / 8] >> (i % 8)) & 1;
		}
...
{
	int i;
	BitSequence intermediate[N * 3 + 8];
	...
	if (doSmooth) 
	{
		...
		for (i = 0; i < (N + 1) * 8; ++i) 
		{
			if (outputBits[i] == 1) 
			{
				...
				const int16_t *AsRow = As + N * M + ((i / N) * N);  
				...
				for (j = 0; j < N; ++j) 
				{
					if (j < AShift) 
					{
						output[j] -= AsRow[N + j - AShift];
					}
					else 
					{
						output[j] += AsRow[j - AShift];
					}
				}
			}
		}
