...
void FFT(const BitSequence input[EIGHTH_N], int32_t *output);
...
int16_t TranslateToBase256(int32_t input[EIGHTH_N], BitSequence output[EIGHTH_N])
{
	...
	for (i = (EIGHTH_N / 2) - 1; i > 0; --i)
	{
		...
		for (j = i - 1; j < (EIGHTH_N / 2) - 1; ++j) 
		{
			...
			register int32_t temp = pairs[j] + pairs[j + 1] + (pairs[j + 1] << 9);
			pairs[j] = temp & 0xffff;
			pairs[j + 1] += (temp >> 16);
		}
	}
...
void FFT(const BitSequence input[EIGHTH_N], int32_t *output)
{
	register int16_t *mult = multipliers;
	register int32_t F0, F1, F2, F3, F4, F5, F6, F7, F8, F9,
	...
	register int16_t *table = &(fftTable[input[0] << 3]);
...
void SWIFFTFFT(const BitSequence *input, int m, int32_t *output)
{
	int i;
...
void SWIFFTSum(const int32_t *input, int m, BitSequence *output, const int16_t *a) 
{ 
	int i, j;
	int32_t result[N];
	register int16_t carry = 0;
