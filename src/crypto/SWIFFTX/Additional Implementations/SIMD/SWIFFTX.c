...
//  October 2008
//
//  This is the source file of the SIMD OPTIMIZED implementation of SWIFFTX hash function.
//  For use on Intel or other processors supporting SSE SIMD.
...
#define LOGN 6   // N size in bits
#define P 257    // Modulus. Arithmetic is performed modulo P. 

//
// This M is used for the first 2 SWIFFTS done on the input
//
#define M (SWIFFTX_INPUT_BLOCK_SIZE/8)   // 32
//
// This is the M used for the final SWIFFT (done on the output of the
// first 3 SWIFFTS)
//
#define FINAL_M (3 * (N/8) + 1)

#define OMEGA 42 // Generator of multiplicative subgroup

#define W 8      // Number of coordinates in SSE2 register

typedef int16_t Zvec  __attribute__ ((vector_size (W*sizeof(int16_t)))); 

#define ALIGN __attribute__ ((aligned (16)))


int16_t multipliers[N] ALIGN;
int16_t fftTable[256*(N/8)] ALIGN;


// The key (A's) we use in SWIFFTX shall be random elements of Z_257.
...
int16_t keysum0[N] ALIGN;
int16_t keysum1[N] ALIGN;
int16_t keysum2[N] ALIGN;
int16_t keysum3[N] ALIGN;
//
// Here starts out SIMD optimized code for the SWIFFT
//
#define INLINE inline extern __attribute__((always_inline))

// Define inline functions CMPEQ, SHIFTL and SHIFTR that perform 
// ==, << and >> vector operations not yet supported by gcc.
// Only works with Zvec=V8SI (i.e., W=8, Z=int16_t). */
INLINE Zvec CMPEQ(Zvec v, Zvec u) { return (__builtin_ia32_pcmpeqw128(v,u)); }
INLINE Zvec SHIFTL(Zvec v, int s) { return (__builtin_ia32_psllwi128(v,s)); }
INLINE Zvec SHIFTR(Zvec v, int s) { return (__builtin_ia32_psrawi128(v,s)); }
#define CONST(n) ((Zvec) {n,n,n,n,n,n,n,n})


// (x*2^s mod 256) - floor(x/2^{8-s})
// This effectively multiplies input by  2^s mod P=257 while 
// reducing the range by approximately (8-s) bits
INLINE Zvec shift(Zvec x, int s) 
{ return (SHIFTL(x,s) & CONST(255)) - SHIFTR(x,8-s); }

// Reduces mm mod P=257 to the range {-128+1,3*128-1}
INLINE Zvec qReduce(Zvec x) // (x mod 256) - floor(x/256) 
{ return (x & CONST(255)) - SHIFTR(x,8); } 

// Reduces mm mod P=257 to the range {0,..,P-1}
INLINE Zvec modP(Zvec mm)
{
  Zvec tmp = qReduce(qReduce(mm));  
  return tmp ^ (CMPEQ(tmp,CONST(-1)) & CONST(-257)); 
}

INLINE Zvec packhighs(Zvec a1,Zvec a2) { return  __builtin_ia32_punpckhwd128 (a1,a2); }
INLINE Zvec packlows(Zvec a1,Zvec a2) { return  __builtin_ia32_punpcklwd128 (a1,a2); }

#define AddSub(a, b) { Zvec tmp = b; b = a - b; a = a + tmp; }

typedef uint8_t Bvec  __attribute__ ((vector_size (16*sizeof(int8_t)))); 

#define CONSTB(n) ((Bvec) {n,n,n,n,n,n,n,n,n,n,n,n,n,n,n,n})
...
//
// This routine pre-process the key, in a way that is needed for the SIMD calculation
// Explanation:
// This preprocessing is neccessary to avoid overflow when multiplying by key. 
// In the SIMD version we have an expresion like (qReduce(x)-(P-1)/2)*key_val .
// The key must be 'centered' (ie be between -128 and 128), and the (P-1)/2 subtractuion is neccessary
// so that the maximum absolute value of the multiplicand is 255.
// That neccesiates the calculation of keysum , which is  (P-1)/2 * (sum of the keys in each coloumn).
//
void KeysumInit(int16_t *key, int m,int16_t keysum[N]) 
{
  // First center our key
  int i,j;
  for (j=0; j<N*m; j++) {
    key[j]=Center(key[j]);
  }
  for (j=0; j<N; j++) {
    long sum = 0;
    int16_t *kcol=key+j;
    for (i=0; i<m; i++,kcol+=N) {
      sum += *kcol;
    }
    keysum[j] = Center(sum * ((P-1)/2));
  }
}
...
void InitializeSWIFFTX() 
{
	int i, j, k, x;
	// The powers of OMEGA
	int omegaPowers[2 * N + 1]; 
    int8_t reverseBits[N];
	omegaPowers[0] = 1;
	...
	for (i = 0; i < N/W; ++i) {
        reverseBits[i]= ReverseBits(i,N/W);
    }
	...
	for (i = 0; i < (N / W); ++i)
	{
		for (j = 0; j < W; ++j)
		{
			multipliers[(i << 3) + j] = omegaPowers[reverseBits[i] * (2 * j + 1)];
		}
	}

	for (i = 0; i < W; ++i) {
        reverseBits[i]= ReverseBits(i,W);
    }
	...
	for (x = 0; x < 256; ++x) 
	{
		for (j = 0; j < 8; ++j) 
		{
			...
			for (k = 0; k < 8; ++k)
			{
				temp += omegaPowers[((N/8) * (2 * j + 1) * reverseBits[k]) % (2 * N)] 
					  * ((x >> k) & 1);
			}
    ...
    //
    // Initialize smooth_key by just coverting to uint_t.
    // Becuse of the ugly side-effect of KeysumInit (centering of PI_key) it
    // is important to do it now and not after centering ...
    //
    for(i=0;i<9;i++) {
        for(j=0;j<N;j++) {
            Smooth_key[i*N+j]=PI_key[i*N+j+M*N];
        }
    }
    //
    // initialize the keysums we need
    //
    KeysumInit(PI_key, M, keysum0);
    KeysumInit(PI_key+M*N, M, keysum1);
    KeysumInit(PI_key+2*M*N, M, keysum2);
    KeysumInit(PI_key, FINAL_M, keysum3);
...
//
//  vals array is assumed to have (W=8) N8=8 digits in base 257. 
//  Assume that most significiant is last.
//  output in ovals is the same 8 numbers encoded in base 256.
//
Zvec ToBase256( Zvec vals[8])
{
  int i;
  for(i=8-1;i>0;i--){
    int j;
    for(j=i-1;j<8-1;j++) {
      Zvec v=vals[j]+vals[j+1];
      vals[j]=v & CONST(255);
      vals[j+1]+=SHIFTR(v,8);
    }
  }
...
//  keysum is an auxiliary array of size N prepared by setup_keysum.
//  This routine also includes in the end the base change algorithm
//  which converts 64 values in 0:257 to 65 bytes.
//  notice that ikey , ikeysum are cast to Zvec , so they must be
//  properly aligned ....
//
void SWIFFT_fft(const BitSequence *input, int m, int16_t *fftout)
{
  int i,j;
  Zvec *out=(Zvec *)fftout;
  Zvec *Mult = (Zvec*) multipliers;
  Zvec *Tabl = (Zvec*) fftTable;

  const BitSequence *t=input;
  for (i=0; i<m; i++,t+=8,out+=8) {
    Zvec v0 = Tabl[t[0]];
    Zvec v1 = Tabl[t[1]] * Mult[1];
    Zvec v2 = Tabl[t[2]] * Mult[2];
    Zvec v3 = Tabl[t[3]] * Mult[3];
    Zvec v4 = Tabl[t[4]] * Mult[4];
    Zvec v5 = Tabl[t[5]] * Mult[5];
    Zvec v6 = Tabl[t[6]] * Mult[6];
    Zvec v7 = Tabl[t[7]] * Mult[7];
    ...
    out[0] = (qReduce(v0) - CONST((P-1)/2));
    out[1] = (qReduce(v1) - CONST((P-1)/2));
    out[2] = (qReduce(v2) - CONST((P-1)/2));
    out[3] = (qReduce(v3) - CONST((P-1)/2));
    out[4] = (qReduce(v4) - CONST((P-1)/2));
    out[5] = (qReduce(v5) - CONST((P-1)/2));
    out[6] = (qReduce(v6) - CONST((P-1)/2));
    out[7] = (qReduce(v7) - CONST((P-1)/2));
  }
}

void SWIFFT_sum(const int16_t *ikey, const int16_t *ikeysum, 
		        const  int16_t *ifftout, int m, BitSequence *hash) 
{
  int i;
  const Zvec *key=(const Zvec *)ikey;
  const Zvec *keysum=(const Zvec *)ikeysum;
  const Zvec *fftout=(const Zvec *)ifftout;
  ...
  for (i=0; i<m; i++,fftout+=8,key+=8) {
    O0 += qReduce(fftout[0] * key[0]);    
    O1 += qReduce(fftout[1] * key[1]);    
    O2 += qReduce(fftout[2] * key[2]);    
    O3 += qReduce(fftout[3] * key[3]);    
    O4 += qReduce(fftout[4] * key[4]);    
    O5 += qReduce(fftout[5] * key[5]);    
    O6 += qReduce(fftout[6] * key[6]);    
    O7 += qReduce(fftout[7] * key[7]);    
  }
  Zvec out[N/W],transposed[N/W];
  out[0] = modP(O0);
  out[1] = modP(O1);
  out[2] = modP(O2);
  out[3] = modP(O3);
  ...
  //
  int16_t *tin=(int16_t *) out;
  int16_t *tout=(int16_t *) transposed;
  for (i=0; i<N/W; i++,tin+=8,tout++) {
    tout[0]=tin[0];
    tout[8]=tin[1];
    tout[16]=tin[2];
    tout[24]=tin[3];
  ...
  BitSequence *cout=hash;
  int carry=0;
  for (i=0; i<N/W; i++,tin++,cout+=8) {
    //printf("cout is %x\n",cout);
    cout[0]=tin[0];
    cout[1]=tin[8];
    cout[2]=tin[16];
...
// uncomment to use with gcc 4.3.2
//#define GCC_4_3_2
#ifdef GCC_4_3_2
//
// This code could not run for me on GCC 4.2.1 somehow gcc chokes with pslldq (double quad) shifts
// So We have to resort to the less efficient and dirtier code in the #else part
// This code however compiled and run correctly on GCC 4.3.2 under cygwin
//
INLINE Bvec roll_right(Bvec v, int s) { return (__builtin_ia32_pslldqi128(v,s)); }
INLINE Bvec roll_left(Bvec v, int s) { return (__builtin_ia32_psrldqi128(v,s)); }

void SWIFFT_smooth(BitSequence output[N+1])
{
  Bvec sum[4] ALIGN;
  Bvec *key=(Bvec *)Smooth_key;
  int i;

  Bvec sum0=CONSTB(0),sum1=CONSTB(0),sum2=CONSTB(0),sum3=CONSTB(0);
  Bvec key0,key1,key2,key3;2

  for (i = 0; i < (N+1)*8; ++i) {

    if((i&63)==0) { // read the input row
      key0=key[0];
      key1=key[1];
      key2=key[2];
      key3=key[3];
    ...
    // SIMD roll of key ..
    //
    Bvec carry=roll_left(key3,15*8);
    key3=roll_right(key3,8)+roll_left(key2,15*8);
    key2=roll_right(key2,8)+roll_left(key1,15*8);
    key1=roll_right(key1,8)+roll_left(key0,15*8);
    key0=roll_right(key0,8)-carry;
  }
  sum[0]=sum0;
  sum[1]=sum1;
  sum[2]=sum2;
  sum[3]=sum3;
  memcpy(output,sum,N);  // This is necessary if output was not aligned
  output[N]=0;
}

#else
//
// We had to write more cumbersome and less efficient code because gcc 4.2.1 seem to
// encounter a strange internal bug with doublequad shift so this one uses quadshift which
// is OK with that gcc.
//
INLINE Bvec roll_right(Bvec v, int s) { return (__builtin_ia32_psllqi128(v,s)); }
...
void SWIFFT_smooth(BitSequence output[N+1])
{
  ...
  Bvec sum0=CONSTB(0),sum1=CONSTB(0),sum2=CONSTB(0),sum3=CONSTB(0);
  Bvec key0,key1,key2,key3;

  for (i = 0; i < (N+1)*8; ++i) {

    if((i&63)==0) { 
      // read the input key row
      key0=key[0];
      key1=key[1];
      key2=key[2];
      key3=key[3];
    ...
    //
    // SIMD roll of key ..
    // Here is the ugly part as we use memory addressing 
    // to hack around the absence of double quad shift.
    //
    uint8_t carrybyte=((uint8_t *)&key3)[15];
    uint8_t lowbyte=((uint8_t *)&key2)[15];
    uint8_t middlebyte=((uint8_t *)&key3)[7];
    key3=roll_right(key3,8);
    ((uint8_t *)&key3)[8] = middlebyte;
    (*(uint8_t *)&key3) = lowbyte;

    lowbyte=((uint8_t *)&key1)[15];
    middlebyte=((uint8_t *)&key2)[7];
    key2=roll_right(key2,8);
    ((uint8_t *)&key2)[8] = middlebyte;
    (*(uint8_t *)&key2) = lowbyte;

    lowbyte=((uint8_t *)&key0)[15];
    middlebyte=((uint8_t *)&key1)[7];
    key1=roll_right(key1,8);
    ((uint8_t *)&key1)[8] = middlebyte;
    (*(uint8_t *)&key1) = lowbyte;

    middlebyte=((uint8_t *)&key0)[7];
    key0=roll_right(key0,8);
    ((uint8_t *)&key0)[8] = middlebyte;
    (*(uint8_t *)&key0) = -carrybyte;
  }
  sum[0]=sum0;
  sum[1]=sum1;
  sum[2]=sum2;
...
void ComputeSingleSWIFFTX(BitSequence input[SWIFFTX_INPUT_BLOCK_SIZE], 
                          BitSequence output[SWIFFTX_OUTPUT_BLOCK_SIZE], bool doSmooth)
{
  int i;
  int16_t fftout[N*M] ALIGN;
  BitSequence intermediate[3*N+8];
  BitSequence carry0,carry1,carry2;

  SWIFFT_fft(input, M, fftout);
  SWIFFT_sum(PI_key,keysum0, fftout, M, intermediate);
  carry0 = intermediate[N];  

  SWIFFT_sum(PI_key+M*N,keysum1, fftout, M, intermediate + N);
  carry1 = intermediate[2 * N];  

  SWIFFT_sum(PI_key+2*M*N, keysum2, fftout, M, intermediate + 2*N);
  ...
  SWIFFT_fft(intermediate, FINAL_M, fftout);
  SWIFFT_sum(PI_key,keysum3, fftout, FINAL_M, output);
  if(doSmooth) {
    SWIFFT_smooth(output);
  }
}
